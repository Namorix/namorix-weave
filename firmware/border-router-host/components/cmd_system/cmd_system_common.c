...
static int get_version(int argc, char **argv)
{
    ...
    printf("Chip info:\r\n\tmodel:%s\r\n\tcores:%d\r\n", model, info.cores);
    printf("\tfeature:%s%s%s%s%" PRIu32 " MB\r\n\trevision:%d\r\n",
           info.features & CHIP_FEATURE_WIFI_BGN ? "/802.11bgn" : "",
           info.features & CHIP_FEATURE_BLE ? "/BLE" : "",
           info.features & CHIP_FEATURE_BT ? "/BT" : "",
           info.features & CHIP_FEATURE_EMB_FLASH ? "/Embedded-Flash:" : "/External-Flash:",
           flash_size / (1024 * 1024), info.revision);
    return 0;
}
...
static void register_version(void)
{
    const esp_console_cmd_t cmd = {
        .command = "version", .help = "Get version of chip and SDK",
        .hint = NULL, .func = &get_version,
    };
...
{
    const esp_console_cmd_t cmd = {
        .command = "restart", .help = "Software reset of the chip",
...
{
    const esp_console_cmd_t cmd = {
        .command = "free", .help = "Get the current size of free heap memory",
        .hint = NULL, .func = &free_mem,
    };
    ESP_ERROR_CHECK(esp_console_cmd_register(&cmd));
}
...
{
    const esp_console_cmd_t cmd = {
        .command = "tasks", .help = "Get information about running tasks",
        .hint = NULL, .func = &tasks_info,
    };
    ESP_ERROR_CHECK(esp_console_cmd_register(&cmd));
}
#endif
...
static int log_level(int argc, char **argv)
{
    ...
    if (level > CONFIG_LOG_MAXIMUM_LEVEL) {
        printf("Can't set log level to %s, max in menuconfig: %s\n",
...
static void register_log_level(void)
{
    ...
    log_level_args.level = arg_str1(NULL, NULL, "<none|error|warn|info|debug|verbose>", "Log level");
    log_level_args.end = arg_end(2);
    const esp_console_cmd_t cmd = {
        .command = "log_level", .help = "Set log level for all tags or a specific tag.",
        .hint = NULL, .func = &log_level, .argtable = &log_level_args
    };
    ESP_ERROR_CHECK(esp_console_cmd_register(&cmd));
}

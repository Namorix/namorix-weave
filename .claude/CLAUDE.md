...
Scaffold + Network monitoring UI now fed by live Border-Router data over SignalR. Session auth (OAuth user login qua desktop) + single-origin dev proxy added in v0.4.0; refactored into sibling `@namorix/core` `AddonSessionAuth` in v0.4.1 (endpoints `/api/oauth/*` + cookie `nmx_weave_session` giữ nguyên, controller chỉ expose khi `AddAddonSessionAuth<TContext>()` được gọi). In v0.4.2 the dev Vite proxy + Chrome DevTools 404 moved into `@namorix/core` extensions (`AddDevViteReverseProxy`/`MapDevViteReverseProxy`/`UseChromeDevToolsProbe404` — bỏ YARP local), `useSessionGuard` returns a state (`"loading"|"authenticated"|"unauthorized"` — `WeaveApp` gates render with `NmxLoadingOverlay`), and `BrConnectionService` DI was fixed (`AddSingleton` + `AddHostedService(factory)` so `WeaveHub` resolves it). In v0.5.0 the **BR provisioning UI "Add border router" (Batch 3)** landed — `NetworkView` is now a network list (`BrProvisionPanel`, `NmxGrid` cards with Accept/Reject dialogs) and click-through opens `ThreadNetworkView` (detail, 4 sub-tabs, back via `onBack`). Client→backend mutations (Accept/Reject) moved from SignalR hub invoke to **REST** (`Controllers/NetworkController.cs` → `POST /api/networks/{id}/accept|reject`, frontend `controllers/network.controller.ts` qua `coreConfig.http`) — SignalR `/hubs/weave` only pushes (`border-router:*`), theo convention namorix. **In v0.6.0 (Batch 3.5) the hub became notify-only** — notifier layer tách khỏi `WeaveHub` (`Infrastructure/INetworkNotifier` + `IBrNotifier`, `Hubs/SignalRNetworkNotifier` + `SignalRBrNotifier` push `Clients.Group`), `WeaveHub` chỉ còn `SubscribeNetwork`/`UnsubscribeNetwork` (group `network`), `BrConnectionService` bỏ `IHubContext`, mutation hẳn về REST kèm snapshot `GET /api/networks/{id}/br/state|dataset|tables`; `NetworkView` danh sách inline (xoá `BrProvisionPanel`); `coreConfig` bỏ `createAuthRefresh` desktop (vòng lặp `/api/auth/refresh` → YARP catch-all "Header overflow") thay bằng probe `/api/oauth/status` + redirect `/api/oauth/login`; `protocol`/`status` API trả lowercase. **In v0.6.1 (2026-08-19)** `NetworkView` được wrap vào `NmxToolbarContainer` (mới, sibling `@namorix/ui`) + thêm `NmxButtonRefresh` (i18n `network.refresh`); fix React Hooks order trong `WeaveApp` (hoist `useAddonMode` lên đầu component) và fix subscribe race ở `@namorix/core` `useSignalRGroup` (đợi connection `"connected"` trước khi invoke).

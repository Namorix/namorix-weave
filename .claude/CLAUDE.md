...
Scaffold + Network monitoring UI now fed by live Border-Router data over SignalR. Session auth (OAuth user login qua desktop) + single-origin dev proxy (YARP → Vite) added in v0.4.0; refactored into sibling `@namorix/core` `AddonSessionAuth` in v0.4.1 (endpoints `/api/oauth/*` + cookie `nmx_weave_session` giữ nguyên, controller chỉ expose khi `AddAddonSessionAuth<TContext>()` được gọi).
